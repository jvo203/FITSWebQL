...
#define VERSION_STRING "SV2019-10-06.0"
...
sqlite_callback(void *userp, int argc, char **argv, char **azColName)
{
    ...
    if (argc == 8)
    {
        ...
        if (stream->compress)
        {
            ...
            do
            {
                ...
                if (have > 0)
                {
                    ...
                    chunk << std::hex << have << "\r\n";
                    stream->res->Super::write(chunk.str().c_str(), chunk.tellp());

                    //chunk contents
                    stream->res->Super::write((const char *)stream->out, have);
                    stream->res->Super::write("\r\n", 2);
                }
        ...
        else
        {
            ...
            //chunk contents
            stream->res->Super::write(json.c_str(), json.length());
            stream->res->Super::write("\r\n", 2);
        }
    }
...
void stream_molecules(auto *res, double freq_start, double freq_end, bool compress)
{
    if (splat_db == NULL)
    ...
    {
        {
            ...
            res->Super::write(chunk.str().c_str(), chunk.tellp());
        }
        ...
        return new std::thread([]() {
            ...
            uWS::App().get("/*", [](auto *res, auto *req) {
                ...
                if (uri.find("/get_directory") != std::string::npos)
                {
                    ...
                    if (pos != std::string::npos)
                    {
                        std::string_view query = uri.substr(pos + 1, std::string::npos);
                ...
                .open = [](auto *ws, auto *req) {
                    //ws->subscribe("opening a websocket connection");
                    std::string_view url = req->getUrl();
                    std::cout << "[µWS] onConnection URL(" << url << ")" << std::endl;
                },
                .close = [](auto *ws, int code, std::string_view message) {                    
                    std::cout << "[µWS] onDisconnection " << message << std::endl;
                },
                ...
                if (token) {
                    std::cout << "Thread " << std::this_thread::get_id() << " listening on port " << server_port << std::endl;
                } else {
                    std::cout << "Thread " << std::this_thread::get_id() << " failed to listen on port " << server_port << std::endl;
                }
            }).run();
