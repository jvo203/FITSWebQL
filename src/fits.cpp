...
#pragma omp single
      {
        for (int idy = start_y; idy <= end_y; idy++)
        {
          ...
          for (int idx = start_x; idx <= end_x; idx++)
          {
            ...
            _idx = idx;
            _idy = idy;
#pragma omp task private(_idx, _idy)
            {
...
#pragma omp parallel for shared(start_x, end_x, start_y, end_y) schedule(dynamic, 4)
  for (size_t i = (start - (start % 4)); i <= end; i++)
...
bool scan_fits_header(struct FITSDownloadStruct *download, const char *contents, size_t size)
{
  ...
  if (end)
  {
    ...
    if (fits->depth > 1)
    {
      ...
      // there is only one histogram thread when receiving FITS data sequentially
      fits->hist_pool.resize(1);
...
void scan_fits_data(struct FITSDownloadStruct *download, const char *contents, size_t size)
{
  ...
  if (fits->depth > 1)
  {
    ...
    if (frame_end && start_k >= 0)
    {
      ...
      else if (frame == fits->depth - 1)
      {
        // tidy up the loose (odd) ends
        start_k = frame - (frame % 4);
        std::cout << "zfp_compress_cube(" << start_k << ")" << std::endl;
        fits->zfp_compress_cube(start_k);
      }
    }
