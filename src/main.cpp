...
#define VERSION_STRING "SV2019-05-09.0"
...
void stream_molecules(uWS::HttpResponse *res, double freq_start,
...
void http_fits_response(uWS::HttpResponse *res,
...
void execute_fits(uWS::HttpResponse *res, std::string dir, std::string ext,
          ...
          h.onHttpRequest([](uWS::HttpResponse *res, uWS::HttpRequest req,
            ...
            if (uri.find("FITSWebQL.html") != std::string::npos) {
              ...
              if (pos != std::string::npos) {
                ...
                //(works OK with v19 Update 2, which cannot be installed on
                //CentOS 6) LLVM CLANG works OK with std::string_view
                ...
                if (datasets.size() == 0) {
                  const std::string not_found =
                      "ERROR: please specify at least one dataset in the URL "
                      "parameters list.";
