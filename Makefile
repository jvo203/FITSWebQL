...
dev:
	icpc -g -O3 -xCORE-AVX2 -mcmodel large -qopenmp -qopt-streaming-stores auto -funroll-loops -ipo -std=c++17 -fp-model fast -DHAVE_INLINE -DDEVELOPMENT -qopt-report=5 -qopt-report-phase=vec $(INC) $(SRC) -o $(TARGET) -lstdc++fs -ipp -lsqlite3 -lcurl -lpq -luWS -lssl -lz -lnuma -lpthread -L`jemalloc-config --libdir` -Wl,-rpath,`jemalloc-config --libdir` -ljemalloc `jemalloc-config --libs`
